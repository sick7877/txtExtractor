...
import urllib.parse import requests import json from pyrogram import Client, filters from pyrogram.types import Message import cloudscraper import os

@Client.on_message(filters.command(["exampur"]) & ~filters.edited) async def account_login(bot: Client, m: Message): await m.reply_text("Send ID & Password in this format: email*password")
...
payload = json.dumps({"phone_ext": "91", "phone": "", "email": email, "password": password})
...
subj_res = scraper.get(f"https://auth.exampurcache.xyz/course_subject/{batch_id}", headers=auth_headers).content
subj_data = json.loads(subj_res)["data"]

subj_ids = "".join([f"{s['_id']}&" for s in subj_data])
subj_ids = subj_ids.strip('&')
...
try:
    for subj_id in subj_input:
        chapter_res = requests.get(f"https://auth.exampurcache.xyz/course_material/chapter/{subj_id}/{batch_id}", headers=auth_headers)
        ...
        for chapter in chapters:
            ...
            material_res = requests.get(f"https://auth.exampurcache.xyz/course_material/material/{subj_id}/{batch_id}/{enc_chap}", headers=auth_headers)
